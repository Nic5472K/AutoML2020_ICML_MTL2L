...
###===###
if 1:
    PLHDR = torch.load('A004b_AutoML_Data.FashionMNIST_SGD')
...
#---
if 1:
    PLHDR = torch.load('A004c_AutoML_Data.FashionMNIST_SGDm')
    PLHDR = torch.cat(PLHDR, dim = 1)
    PLHDR = torch.mean(PLHDR, dim = 1).numpy()
    LE10_SGDm = PLHDR
...
#---
if 1:
    PLHDR = torch.load('A004d_AutoML_Data.FashionMNIST_Adam')
    PLHDR = torch.cat(PLHDR, dim = 1)
    PLHDR = torch.mean(PLHDR, dim = 1).numpy()
    LE10_Adam = PLHDR
...
#---
if 1:
    PLHDR = torch.load('A004e_AutoML_Data.FashionMNIST_LSTMoptimiser')
...
#---
if 1:
    #PLHDR = torch.load('D003_LE10_all.FashionMNIST')
    PLHDR = torch.load('A004f_AutoML_Data.FashionMNIST_MTL2L')
    PLHDR = [torch.tensor(i).unsqueeze(1) for i in PLHDR]
    PLHDR = torch.cat(PLHDR, dim = 1)
    PLHDR = torch.mean(PLHDR, dim = 1).numpy()
    LE10_MTL2L = PLHDR
...
plt.title('Trained on FashionMNIST tested on FashionMNIST', fontsize=15)
plt.show()
