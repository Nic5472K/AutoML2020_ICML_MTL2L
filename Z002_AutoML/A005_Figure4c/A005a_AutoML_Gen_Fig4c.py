...
###===###
if 1:
    PLHDR = torch.load('A005b_AutoML_Data.KMNIST_SGD')
...
#---
if 1:
    PLHDR = torch.load('A005c_AutoML_Data.KMNIST_SGDm')
    PLHDR = torch.cat(PLHDR, dim = 1)
    PLHDR = torch.mean(PLHDR, dim = 1).numpy()
    LE10_SGDm = PLHDR
...
#---
if 1:
    PLHDR = torch.load('A005d_AutoML_Data.KMNIST_Adam')
    PLHDR = torch.cat(PLHDR, dim = 1)
    PLHDR = torch.mean(PLHDR, dim = 1).numpy()
    LE10_Adam = PLHDR
...
#---
if 1:
    PLHDR = torch.load('A005e_AutoML_Data.KMNIST_LSTMoptimiser')
...
#---
if 1:
    PLHDR = torch.load('A005f_AutoML_Data.KMNIST_MTL2L')
    PLHDR = [torch.tensor(i).unsqueeze(1) for i in PLHDR]
    PLHDR = torch.cat(PLHDR, dim = 1)
    PLHDR = torch.mean(PLHDR, dim = 1).numpy()
    LE10_MTL2L = PLHDR
...
plt.title('Trained on KMNIST tested on KMNIST', fontsize=15)
plt.show()
