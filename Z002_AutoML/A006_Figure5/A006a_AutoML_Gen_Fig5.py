...
###===###
if 1:
    PLHDR = torch.load('A006b_AutoML_Data.ToyCifar10_SGD')
...
#---
if 1:
    PLHDR = torch.load('A006c_AutoML_Data.ToyCifar10_SGDm')
    PLHDR = torch.cat(PLHDR, dim = 1)
    PLHDR = torch.mean(PLHDR, dim = 1).numpy()
    LE10_SGDm = PLHDR
...
#---
if 1:
    PLHDR = torch.load('A006d_AutoML_Data.ToyCifar10_Adam')
    PLHDR = torch.cat(PLHDR, dim = 1)
    PLHDR = torch.mean(PLHDR, dim = 1).numpy()
    LE10_Adam = PLHDR
...
#---
if 1:
    PLHDR = torch.load('A006e_AutoML_Data.ToyCifar10_LSTMoptimiser')
...
#---
if 1:
    PLHDR = torch.load('A006f_AutoML_Data.ToyCifar10_MTL2L')
    PLHDR = [torch.tensor(i).unsqueeze(1) for i in PLHDR]
    PLHDR = torch.cat(PLHDR, dim = 1)
    PLHDR = torch.mean(PLHDR, dim = 1).numpy()
    LE10_MTL2L = PLHDR
...
plt.title('Trained on MNISTs tested on Modified Cifar10', fontsize=15)
plt.show()
